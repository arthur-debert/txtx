...
suite('Simple Integration Test Suite', () => {
  ...
  test('Commands should be registered', async () => {
    ...
    assert.ok(commands.includes('rfcdoc.formatDocument'), 'formatDocument command should be registered');
    assert.ok(commands.includes('rfcdoc.generateTOC'), 'generateTOC command should be registered');
  });
});